...
//this class adopts both the central and peripheral delegates
...
final class BluetoothSerial: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    let centralQueue: DispatchQueue = DispatchQueue(label: "com.alerrt", attributes: .concurrent)
    ...
    weak var writeCharacteristicHead: CBCharacteristic?
    weak var writeCharacteristicChest: CBCharacteristic?
    ...
    func sendMessageToDevice(_ message: String) {
        ...
        if let data = message.data(using: String.Encoding.utf8) {
            for peripheral in connectedPeripherals{
                if peripheral.name == "HEAD" {
                    peripheral.writeValue(data, for: writeCharacteristicHead!, type: writeType)
                }
                if peripheral.name == "CHEST" {
                    peripheral.writeValue(data, for: writeCharacteristicChest!, type: writeType)
                }
            }
        }
    }
    ...
    func sendBytesToDevice(_ bytes: [UInt8]) {
        ...
        connectedPeripheral!.writeValue(data, for: writeCharacteristicHead!, type: writeType)
    }
        ...
        for characteristic in service.characteristics! {
            if characteristic.uuid == characteristicUUID {
                ...
                if peripheral.name == "HEAD"{
                    ...
                    writeCharacteristicHead = characteristic
                }
                if peripheral.name == "CHEST"{
                    writeCharacteristicChest = characteristic
                }
                // find out writeType
